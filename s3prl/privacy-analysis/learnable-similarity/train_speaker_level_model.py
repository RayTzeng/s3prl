...
def main(args):
    ...
    train_dataset = SpeakerLevelDataset(args.base_path, seen_splits, unseen_splits, AUXILIARY_DATA_SIZE, args.model)
    eval_dataset = SpeakerLevelDataset(args.base_path, seen_splits, unseen_splits, AUXILIARY_DATA_SIZE, args.model)
    ...
    while epoch < args.n_epochs:
        ...
        for batch_id, (features_x, features_y, labels, speakers) in enumerate(tqdm(train_dataloader, dynamic_ncols=True, desc=f'Train | Epoch {epoch}')):
            ...
            optimizer.step()
            if batch_id % 25 == 0:
                str_pred = ",".join([f"{x:.4f}" for x in pred.detach().cpu().tolist()[:10]])
                str_label = ",".join([f"{x:.4f}" for x in labels.detach().cpu().tolist()[:10]])
        ...
        if total_loss < min_loss:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_path, f'learnable-similarity-{args.model}-seed-{args.seed}.pt'))
            early_stopping = 0
        else:
