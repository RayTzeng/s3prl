...
def main(args):
    ...
    unseen_split_pathes = [os.path.join(args.base_path, split) for split in unseen_splits]
    ...
    x = [1, CHOICE_SIZE+1, CHOICE_SIZE*((N+1)/N)+1, CHOICE_SIZE*((N+2)/N)+1, CHOICE_SIZE*((N+3)/N)+1]
    ticks = np.concatenate((seen_splits, unseen_splits))
    plt.bar(range(1, len(intra_speaker_sim_mean)+1), intra_speaker_sim_mean, color=colors)
    plt.xticks(x, ticks)
    plt.ylabel('Average similarity')
    plt.title("speaker similarity of {}".format(args.model))

    plt.plot([0, len(intra_speaker_sim_mean)+1], [np.mean(intra_speaker_sim_mean[:CHOICE_SIZE]) for _ in range(2)], ls='--', color='blue')
    plt.plot([0, len(intra_speaker_sim_mean)+1], [np.mean(intra_speaker_sim_mean[CHOICE_SIZE:]) for _ in range(2)], ls='--', color='red')
    plt.savefig(os.path.join(args.output_path, f'{args.model}-mean-speaker-sim-bar-plot.png'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_path", help="directory of feature of LibriSpeech dataset")
